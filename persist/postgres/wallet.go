...
func scanEvent(s scanner) (ev wallet.Event, eventID int64, err error) {
	...
	err = s.Scan(&eventID, decode(&ev.ID), &ev.MaturityHeight, &ev.Timestamp, &ev.Index.Height, decode(&ev.Index.ID), &eventType, &eventBuf)
	if err != nil {
		return
	}
...
WHERE event_id = ANY($1::INTEGER[]) AND address_id IN (SELECT address_id FROM wallet_addresses WHERE wallet_id=$2)`

	rows, err := tx.Query(query, pq.Array(eventIDs), id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) AddWallet(w wallet.Wallet) (wallet.Wallet, error) {
	...
	err := s.transaction(func(tx *txn) error {
		...
		return tx.QueryRow(query, w.Name, w.Description, w.DateCreated, w.LastUpdated, encode(w.Metadata)).Scan(&w.ID)
	})
	return w, err
}
...
func (s *Store) UpdateWallet(w wallet.Wallet) (wallet.Wallet, error) {
	...
	err := s.transaction(func(tx *txn) error {
		...
		err := tx.QueryRow(query, w.Name, w.Description, w.LastUpdated, encode(w.Metadata), w.ID).Scan(&dummyID, &w.DateCreated, &w.LastUpdated)
...
func (s *Store) Wallets() (wallets []wallet.Wallet, err error) {
	err = s.transaction(func(tx *txn) error {
		...
		for rows.Next() {
			var w wallet.Wallet
			if err := rows.Scan(&w.ID, &w.Name, &w.Description, &w.DateCreated, &w.LastUpdated, decode(&w.Metadata)); err != nil {
				return fmt.Errorf("failed to scan wallet: %w", err)
			}
			if string(w.Metadata) == "{}" {
				w.Metadata = nil
			}
...
func (s *Store) AddWalletAddress(id wallet.ID, addr wallet.Address) error {
	...
	return s.transaction(func(tx *txn) error {
		...
		// custom encoding for spend policy due to nil support
		var encodedPolicy any
		if addr.SpendPolicy != nil {
			var buf bytes.Buffer
			enc := types.NewEncoder(&buf)
			addr.SpendPolicy.EncodeTo(enc)
			if err := enc.Flush(); err != nil {
				return fmt.Errorf("failed to encode spend policy: %w", err)
			}
			encodedPolicy = buf.Bytes()
		}

		_, err = tx.Exec(`INSERT INTO wallet_addresses (wallet_id, address_id, description, spend_policy, extra_data) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (wallet_id, address_id) DO UPDATE set description=EXCLUDED.description, spend_policy=EXCLUDED.spend_policy, extra_data=EXCLUDED.extra_data`, id, addressID, addr.Description, encodedPolicy, encode(addr.Metadata))
		return err
	})
}
...
WHERE wa.wallet_id=$1`
		...
		for rows.Next() {
			...
			if err := rows.Scan(decode(&address.Address), &address.Description, &decodedPolicy, decode(&address.Metadata)); err != nil {
				return fmt.Errorf("failed to scan address: %w", err)
			}

			if string(address.Metadata) == "{}" {
				address.Metadata = nil
			}

			// custom handling for spend policies due to nil support
...
func (s *Store) WalletSiacoinOutputs(id wallet.ID, offset, limit int) (siacoins []types.SiacoinElement, err error) {
	err = s.transaction(func(tx *txn) error {
		...
		for rows.Next() {
			...
			err := rows.Scan(decode(&siacoin.ID), &siacoin.LeafIndex, decode(&siacoin.MerkleProof), decode(&siacoin.SiacoinOutput.Value), decode(&siacoin.SiacoinOutput.Address), &siacoin.MaturityHeight)
			if err != nil {
...
func (s *Store) WalletSiafundOutputs(id wallet.ID, offset, limit int) (siafunds []types.SiafundElement, err error) {
	err = s.transaction(func(tx *txn) error {
		...
		for rows.Next() {
			...
			err := rows.Scan(decode(&siafund.ID), &siafund.LeafIndex, decode(&siafund.MerkleProof), &siafund.SiafundOutput.Value, decode(&siafund.ClaimStart), decode(&siafund.SiafundOutput.Address))
			if err != nil {
				return fmt.Errorf("failed to scan siacoin element: %w", err)
			}
