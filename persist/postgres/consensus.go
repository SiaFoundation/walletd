...
func (ut *updateTx) AddSiacoinElements(elements []types.SiacoinElement) error {
	...
	for _, se := range elements {
		...
		_, err = insertStmt.Exec(encode(se.ID), encode(se.SiacoinOutput.Value), encode(se.MerkleProof), se.LeafIndex, se.MaturityHeight, addressID)
		if err != nil {
			return fmt.Errorf("failed to execute statement: %w", err)
		}
	}
...
func (ut *updateTx) AddSiafundElements(elements []types.SiafundElement) error {
	...
	for _, se := range elements {
		...
		_, err = insertStmt.Exec(encode(se.ID), se.SiafundOutput.Value, encode(se.MerkleProof), se.LeafIndex, encode(se.ClaimStart), addressID)
		if err != nil {
			return fmt.Errorf("failed to execute statement: %w", err)
		}
	}
...
func (ut *updateTx) AddEvents(events []wallet.Event) error {
	...
	for _, event := range events {
		...
		var eventID int64
		err = eventStmt.QueryRow(encode(event.ID), event.MaturityHeight, event.Timestamp, chainIndexID, event.Data.EventType(), buf.String()).Scan(&eventID)
		if err != nil {
