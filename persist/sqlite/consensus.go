...
func (ut *updateTx) AddressBalance(addr types.Address) (balance wallet.Balance, err error) {
	err = ut.tx.QueryRow(`SELECT siacoin_balance, immature_siacoin_balance, siafund_balance FROM sia_addresses WHERE sia_address=$1`, encode(addr)).Scan(decode(&balance.Siacoin), decode(&balance.Immature), &balance.Siafund)
	return
}
...
func (ut *updateTx) UpdateBalances(balances []wallet.AddressBalance) error {
	...
	for _, ab := range balances {
		_, err := stmt.Exec(encode(ab.Balance.Siacoin), encode(ab.Balance.Immature), ab.Balance.Siafund, encode(ab.Address))
		if err != nil {
			return fmt.Errorf("failed to execute statement: %w", err)
		}
	}
